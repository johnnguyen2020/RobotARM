...
##            self.sockOut.send(self.arm.getIKPose().serialize() + str.encode(sensor))
    ...
    def drawViews(self, pose):
        ...
        self.current = [pose.getServoSwing(), pose.getServoActuator(), pose.getServoElevator()]
        ...
        if len(self.resultList) > 150:
            ...
            if self.resultList[len(self.resultList)-2] != [0.0,0.0,0.0] and self.resultList[len(self.resultList)-1] ==[0.0,0.0,0.0]:
                ...
                self.doneMovingIndex = len(self.resultList)-1
                for i in range(len(self.resultList)-2, -1, -1):
                    if self.resultList[i] == [0.0,0.0,0.0]:
                        self.previousNotMovingIndex = i-1
                self.angleMovement = np.subtract(self.currentList[self.doneMovingIndex], self.currentList[self.previousNotMovingIndex])
